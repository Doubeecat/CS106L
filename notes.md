...
这就涉及到了 **重载函数**。我们在进行函数重载时候，定义的重载函数必须具有不同的参数个数 or 不同的参数类型。这样编译器才会选择最优秀的进行匹配（题外话，如果有模板函数和普通函数同时进入候选列表，编译器会优先选择普通函数进行匹配）。
...
如果双目运算符重载为**成员函数**的时候，是由一个操作数调用了另一个操作数，此时左操作数作为 `this` 指针：
```cpp
A a,b,c;
c = a + b;
//等价于
c = a.operator+(b);
```
但是如果双目运算符重载为**友元函数**的时候，参与运算的对象全部成为函数参数：
```cpp
A a,b,c;
c = a + b;
//等价于
...
对于单目运算符，作为成员函数重载时没有参数，作为友元函数重载时参数为自定义类的对象或引用。

```cpp
class A {
    friend bool operator! (const A & lhs) {
        return lhs->val == 0;
    }
    private :
        int val;
...
所以对于 `<<,>>` 我们只能作为友元函数或者普通函数，并且重载的函数形式如下:
...
记得用引用来修饰运算符，这样才能支持连锁调用，并且可以减少时间空间的开销。
...
补充一些应试考点x：
- `++` 运算符，后自增比前自增在参数列表里多一个 `int`.由于编译器必须能够识别出前缀自增与后缀自增，人为规定用 `operator++()` 重载前置运算符，用 `operator++(int)` 重载后置运算符。
...
如果没有手写拷贝构造函数的时候，编译器会帮助我们生成一个构造函数。但是比如我们的类中含有指针的话，编译器的构造函数只会帮我们复制指针本身，并不会更深地把指针对应的东西拷贝过去，这就需要我们进行手写了。
...
事实上很多时候为了避免假装我们拷贝了某个东西，我们经常是要手写这样的深拷贝。比如之前有一个手写 `vector` 的小项目，个人建议类似这种的不妨先手写一个 `reserve()` 把对应空间搞出来，并且拷贝这种一定要小心考虑。

## 转换构造函数

回顾我们之前经常使用的类型转换，我们也可以实现一个转换构造函数，用于实现其他数据类型转化为自定义类的类型转化。
...
class complex {
    public:
        operator double (const complex &x) {
            return x.real;
        }
    private:
        double real,image;
...
注意 `template<class>` 一般情况下和 `template<typename>` 可以互化，但是实际上更多在定义类的时候用 `class`。
